...
int main()
{
    ...
    int time=0;
    int totalTAT=0,totalBT=0,totalWT=0;
    ...
    {
        // 
        int pid=-1,shortestRT=1e9,earliest=1e9;
        for(int i=0;i<n;i++)
        {
            if(isFinished[i] ||  arr[i].AT>time) continue;
            if(arr[i].RT<shortestRT || (arr[i].RT==shortestRT && arr[i].AT<earliest) || (arr[i].RT==shortestRT && arr[i].AT==earliest && arr[i].pid<pid))
            {
                shortestRT=arr[i].RT;
                earliest=arr[i].AT;
                pid=arr[i].pid;
