...
{
    int pid,AT,BT,CT,ST,TAT,WT,p,RT;
}process;
int main()
{
    int n;
    ...
    while (processFinished<n)
    {
        ...
        if(pid!=-1)
        {
            ...
            printf("%-15d %-15d %-15d %-15d \n",arr[pid].pid, arr[pid].p,arr[pid].AT,arr[pid].BT);
            time++;
            ...
            if(arr[pid].RT==0)
            {
                ...
                printf("%-15d %-15d %-15d %-15d %-15d %-15d %-15d %-15d\n COMPLETED",arr[pid].pid, arr[pid].p,arr[pid].AT,arr[pid].BT,arr[pid].ST,arr[pid].CT,arr[pid].TAT,arr[pid].WT);
